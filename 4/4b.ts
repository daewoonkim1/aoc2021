...
input[0].split(",").forEach(x => {
    ...
    boards.forEach(board => {
        ...
        if (checkBingo(skeleboards[count].data)===true && !winners.includes(board)){
            winners.push(board);
            index.push(count);
        }
        if (winners.length===100){
            console.log(winners[99].data);
            console.log(skeleboards[index[99]].data);
            console.log(sumOfUnmarked(skeleboards[index[99]].data,winners[99].data));
            console.log(x);
            console.log(''); // 266, 21
        }
        count++;
